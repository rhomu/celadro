...
def cell(frame, i, engine=plt, **kwargs):
    ...
    Args:
        ...
        engine: Plotting engine or axis.
        colors: Color to use for the contour.
...
def substrate_phi(frame, engine=plt, cbar=False):
    """
    Plot substrate phi sclar field.
    ...
    """
    cax = engine.imshow(
        frame.substrate_phi.T, interpolation="lanczos", cmap="Greys", origin="lower"
    )
    if cbar:
        plt.colorbar(cax)
